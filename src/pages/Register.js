...
const Register = () => {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setTimeout(() => navigate("/login"), 1500); // new user
      } else if (response.data.alreadyExists) {
        setError("Email already registered. Redirecting to login...");
        setTimeout(() => navigate("/login"), 1500); // already existing user
      } else {
  ...
  const handleAdminLogin = async (e) => {
    ...
    try {
      const response = await axios.post("http://localhost:9090/api/v1/adminlogin", {
        email,
        phone,
  ...
  return (
    <div style={styles.container}>
      <div style={styles.card}>
        ...
        <p onClick={() => navigate('/login')} style={styles.toggle}>
